...
//            List<UserInfo> data = binLog.getData();
            ...
            if ("INSERT".equals(type) && "user".equals(table)) {
                for (int i = 0; i < data.size(); i++) {
                    JSONObject jsonObject = data.getJSONObject(i);
                    redisTemplate.opsForValue().set(jsonObject.getString("username"),
                            JSONObject.parseObject(jsonObject.toJSONString(), User.class),
                            5, TimeUnit.DAYS);
                }
